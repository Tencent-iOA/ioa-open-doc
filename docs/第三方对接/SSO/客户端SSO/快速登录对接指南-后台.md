...
|**参数**|**参数类型**|**参数值**|**参数描述**|
| :- | :- | :- | :- |
|time\_stamp|string|1629968111|时间戳|
|sole\_id|string|58544655|会话ID：随机字符串|
|app\_id|string|eb69748e-47e9-4199-801d-eeb04fc11beb|应用ID，需要在iOA控制台申请|
|reserve|string||预留字段|

示例：
> https://localhost.com:nport/api/public/clientlogin/auth_login?app_id=eb69748e-47e9-4199-801d-eeb04fc11beb&sole_id=58544655&time_stamp=1629968111&reserve=

其中：
localhost：如果是浏览器发起请求，需要合法的证书和域名。如果是第三方终端，也可以使用127.0.0.1。
sole_id：随机值，建议第三方终端每次都不一样，回包时，会带给第三方终端。
app_id：这个需要第三方终端的后台可以配置，以便于适配不同的客户。
请求响应：
...
```
ticket即为获取到的一次性票据。可以用于第三方终端的后台与iOA的后台进行信息交互，确认ticket的有效性和身份信息。

### 3.3.2 第三方后台向iOA后台校验票据
请求方式：POST
请求协议：https
请求地址：iOA总控域名
请求端口：iOA总控服务端口，默认27800
