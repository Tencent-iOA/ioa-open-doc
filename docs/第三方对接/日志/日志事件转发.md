...
| PhishLog | int | 是否上传钓鱼日志 | 1: 是钓鱼日志 |
| PhishRiskTitle | string | 钓鱼告警名称 | |
| PhishDetectEngine | string | 钓鱼告警引擎 | |
| PhishStage | string | 钓鱼告警阶段 | |
| PhishSource | string | 钓鱼来源 | |
| PhishSourceProduct | string | 钓鱼产品 | |
| PhishType | int | 钓鱼类型 | |
| PhishFile | string | 钓鱼文件 | |
| PhishFileMd5 | string | 钓鱼文件md5 | |
| PhishFixResult | int | 钓鱼结果 | |
| PhishUrl | string | 钓鱼外联IP | |
| PhishIP | string | 钓鱼外联地址 | |
| PhishChain | string | 钓鱼攻击链 | |
| PhishDetails | string | 钓鱼详情 | |
