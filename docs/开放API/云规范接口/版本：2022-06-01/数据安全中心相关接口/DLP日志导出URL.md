...
DLP 日志导出URL（小数据量），未超过最大下载日志数据限制时直接返回下载 URL；与 data-mgr/log-center 老接口请求参数保持一致；当前 LogId 仅支持 `DLPAlarmLog`（DLP 审计/拦截日志）。私有化调用path为：`capi/DlpOpenApi/DescribeDLPLogDownloadURL`，从 2026-06-22 版本开始支持。

数据量较大或触发 `FailedOperation.DataTooLarge` 时，请改用异步任务接口 [DLP日志导出任务](/开放API/云规范接口/版本：2022-06-01/数据安全中心相关接口/DLP日志导出任务.md)。
...
| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| Data | [DescribeDLPLogDownloadURLData](/开放API/云规范接口/版本：2022-06-01/数据结构.md#DescribeDLPLogDownloadURLData) | 业务响应数据<br/>注意：此字段可能返回 null，表示取不到有效值。|
...
X-TC-Action: DescribeDLPLogDownloadURL
<公共请求参数>
...
```json
{
    "Response": {
        "Data": {
            "DownloadURL": "https://ioa-dlp-log-1300056258.cos.ap-guangzhou.myqcloud.com/export/DLPAlarmLog_20231124.xlsx?sign=xxxx",
            "DownloadToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.xxxx",
            "ExpireAt": "2023-11-24 12:00:00"
        },
...
```json
{
    "Response": {
        "Data": {
            "DownloadURL": "https://ioa-dlp-log-1300056258.cos.ap-guangzhou.myqcloud.com/export/DLPAlarmLog_intercept_20231124.xlsx?sign=xxxx",
            "DownloadToken": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.xxxx",
            "ExpireAt": "2023-11-24 12:00:00"
        },
        "RequestId": "b8f67971-f7dd-41a8-a42f-caee1df373d9"
...
| 错误码 | 描述 |
|---------|---------|
| FailedOperation.DataTooLarge | 数据量超过最大下载限制，请缩小查询范围或改用异步任务接口 CreateDLPLogDownloadTask。 |
